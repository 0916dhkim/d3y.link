...
- Import ordering
- Suggestions that are purely subjective ("I would prefer...")
- Minor refactors that do not fix a bug or prevent one
- Things that a linter or formatter would catch
- TODOs, FIXMEs, or other markers left intentionally by the author

Only flag issues you are highly confident are real problems. When in doubt, skip it.
...
- If there are NO issues to report and an existing top-level comment indicating "no issues" already exists, do NOT submit another comment. Skip submission to avoid redundancy.
